...
class MyComponent extends san.Component {
  inited() {
    this.data.raw.title = 'MyComponent-Inited';
  }
  initData() {
    return {
      title: 'MyComponent'
  ...
  static computed = {
    myName() {
      const name = this.data.raw.name;
      return `${name} - ${this.data.raw.title}`;
    }
  };
}
...
sanSSRResolver.setRenderer("MyComponent", function  (data, ...info) {
    ...
    let instance = _.createFromPrototype(sanSSRResolver.getPrototype("MyComponent"));
    ...
    data.myName = _.callComputed(ctx, "myName");
...
sanSSRResolver.setRenderer("MyComponent2", function  (data, ...info) {
    ...
    let _output = _.output
    const SanSSRData = sanSSRHelpers.SanSSRData;
    let instance = _.createFromPrototype(sanSSRResolver.getPrototype("MyComponent2"));
    instance.data = new SanSSRData(data, instance);
    instance.sourceSlots = _.mergeChildSlots(slots);
    instance.lifeCycle = {compiled: true, inited: false};
    if (parentCtx) {
