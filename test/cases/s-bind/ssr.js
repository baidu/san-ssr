...
html += "</h3>";
if (componentCtx.data.subtitle) {
html += "<h4>";
html += escapeHTML(componentCtx.data.subtitle);
html += "</h4>";

}
html += "<p>";
html += escapeHTML(componentCtx.data.content);
html += "</p></div>";
return html;
};
...
extend(componentCtx.data.article, {"title":componentCtx.data.title}), true, componentCtx, "x-a", $sourceSlots);
$sourceSlots = null;
html += "<a";
if (componentCtx.data.target) {
html += attrFilter("target", escapeHTML(componentCtx.data.target));
}
(function ($bindObj) {for (var $key in $bindObj) {var $value = $bindObj[$key];
switch ($key) {
case "readonly":
case "disabled":
case "multiple":
